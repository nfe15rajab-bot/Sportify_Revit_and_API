...
| someone else's Combine module changes under **Algorithmic placement** (a zone becomes a polygon, a sport is specified instead of sized, the build-up catalogue moves) with no git conflict, and Apply then makes zones the canvas cannot draw or reserves a court the wrong size | `Sportify/tools/algoplacement-ui-test.js` (run by `run-checks.js`): the real scripts (`zones.js`, the basketball and volleyball modules, `assemblies.js`, the UI) in the browser's order in a sandbox with a DOM that does nothing, the real packing core making a plan, and Apply: zones are polygons the zones layer draws and a saved session restores, a specified court carries its own payload and fills the rectangle the packer reserved (also a half court, which is turned the other way), the catalogue is fetched before the zones are made, and the API being down is said in words. Not a browser: drawing and dragging were tried in one |
...
| `ReferenceDbCheck` | `Sportify.Api`'s `ReferenceDbGuard` (what the API does to `reference.db` at startup) on throw-away database files: a new file; one missing a column or a table (rebuilt, the old file kept as a backup); one that only lacks what the catalog seeders add in place, which is brought up to date with everything entered kept (`CatalogSeeding`); rows entered by hand; a file another program holds open; the three policies. `ReferenceDbCheck --dry-run <reference.db>` says what the guard would do to a real file (tried on a copy) | a catalog built from the current code |
...
| `fixtures/` | the layouts everything runs on, made by `make-fixtures.js` from the bundled sample (`layouts/wind` and `layouts/export` are hand-made: `export` is a real export of the web app with everything the merge of Moamen's design panel added to it, so that what the web app really writes keeps being read by both readers and every analysis); `golden-unity/` are real Unity results | |
...
- **Where a bed's own outline is still its box.** The load, water, rain and sun *numbers* follow a zone's real outline (`zones[].points`) and a piece of furniture is a point load at its catalogue weight (`StructuralCheck`, `SunCheck`, `PercolationCheck` and their oracles cover both). What still uses the zone's bounding box: the wind field's drawing, and where the dynamic analysis's people and rain agents spawn. Those are pictures and starting points, not totals.
