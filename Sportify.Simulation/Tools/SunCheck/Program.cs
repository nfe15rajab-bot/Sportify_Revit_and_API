...
// 8. Kinetics: the louvre's actuation law and its mechanics (LouvreActuationModel, LouvreMechanicsModel), against hand calculations
{
    ...
    Check("deflection goes with the fourth power of the span (3 m against 2 m: 1.5^4), and a 2 m blade passes", Near(rk.DeflectionMm / shortSpan.DeflectionMm, Math.Pow(3.0 / 2.0, 4), 1e-6) && shortSpan.DeflectionOk, $"(ratio {rk.DeflectionMm / shortSpan.DeflectionMm:0.0000})");
    ...
    var ovr = KineticUnits.Overhead(6, 3, 3, 18, 0.15, 0.03, 2, 0.06, 0.04, KineticUnits.NormalOverhead(30, new V3(1, 0, 0)));
    Check("an overhead louvre has a blade per bay per position (18 x 2), posts and rails at every support line (3 lines: 6 posts, 3 rails) a crank on every blade, a rod, an actuator piston and its housing in each bay; the blades, cranks, rods, pistons and their swinging cylinders move, and the hardware is not placed in Revit", ovr.Bars.Count(x => x.Role == "blade") == 36 && ovr.Bars.Count(x => x.Role == "post") == 6 && ovr.Bars.Count(x => x.Role == "rail") == 3 && ovr.Bars.Count(x => x.Role == "rod") == 2 && ovr.Bars.Count(x => x.Role == "crank") == 36 && ovr.Bars.Count(x => x.Role == "piston") == 2 && ovr.Bars.Count(x => x.Role == "housing") == 2 && ovr.Bars.Where(x => x.Dynamic).All(x => x.Role == "blade" || x.Role == "rod" || x.Role == "crank" || x.Role == "piston" || x.Role == "housing") && ovr.Bars.Where(x => !x.Dynamic).All(x => x.Role == "post" || x.Role == "rail") && ovr.Bars.Where(x => x.Role == "crank" || x.Role == "piston" || x.Role == "housing").All(x => x.Detail));
    var blade0 = ovr.Bars.First(x => x.Role == "blade");
    var n30 = KineticUnits.NormalOverhead(30, new V3(1, 0, 0));        // a blade turns about its own axis (along y), so the sun's lean is across it, along x
    ...
    // the mechanism at every blade angle, for the three kinds that have one: each crank is a rigid arm (its length never changes) whose pin lies on the rod, the rod ends where the piston ends,
    // the cylinder is hinged at a point that never moves and stays in line with its piston, and the piston never leaves its cylinder (it reaches at least 5 cm beyond it) or runs out of reach
    (bool Ok, double StrokeM, string Why) Mechanism(Func<double, UnitPlan> build, double[] degrees)
    {
        double? crankLen = null, minReach = double.MaxValue, maxReach = 0; V3? hinge = null; var why = "";
        foreach (var deg in degrees)
        {
            var p = build(deg);
            var rod = p.Bars.First(x => x.Role == "rod"); var pist = p.Bars.First(x => x.Role == "piston"); var hous = p.Bars.First(x => x.Role == "housing");
            var axis = (rod.P1 - rod.P0).Unit();
            foreach (var c in p.Bars.Where(x => x.Role == "crank"))
            {
                var d = c.P1 - rod.P0; var off = d - axis * d.Dot(axis);
                if (off.Length < 1e-6 && d.Dot(axis) > -0.1 && d.Dot(axis) < (rod.P1 - rod.P0).Length + 0.1)     // a crank of this rod (its pin is on the rod's line)
                { var len = (c.P1 - c.P0).Length; crankLen ??= len; if (Math.Abs(len - crankLen.Value) > 1e-9) why = "a crank changes length at " + deg; }
                else if (off.Length < 0.2 && d.Dot(axis) > -0.1 && d.Dot(axis) < (rod.P1 - rod.P0).Length + 0.1 && off.Length > 1e-6) why = "a pin is " + (off.Length * 1000).ToString("0.0") + " mm off the rod at " + deg;
            }
            hinge ??= pist.P0;
            if ((pist.P0 - hinge.Value).Length > 1e-9 || (hous.P0 - hinge.Value).Length > 1e-9) why = "the hinge moves at " + deg;
            if ((pist.P1 - rod.P1).Length > 1e-9) why = "the piston does not reach the rod at " + deg;
            if (Math.Abs((hous.P1 - hous.P0).Unit().Dot((pist.P1 - pist.P0).Unit()) - 1) > 1e-9) why = "the cylinder is not in line with its piston at " + deg;
            var reach = (pist.P1 - pist.P0).Length; minReach = Math.Min(minReach.Value, reach); maxReach = Math.Max(maxReach.Value, reach);
        }
        if (minReach < KineticUnits.CylinderM + 0.05) why = "the piston runs back into its cylinder (reach " + minReach.Value.ToString("0.000") + " m)";
        return (why.Length == 0, maxReach.Value - minReach.Value, why);
    }
    var degs = new[] { -60.0, -30, 0, 30, 60, 75 };
    var mOvr = Mechanism(deg => KineticUnits.Overhead(6, 3, 3, 18, 0.15, 0.03, 2, 0.06, 0.04, KineticUnits.NormalOverhead(deg, new V3(1, 0, 0))), degs);
    var mSlat = Mechanism(deg => KineticUnits.SlatScreen(3, 2.4, 6, 0.4, 0.04, 3, 0.12, 0.08, KineticUnits.NormalSlat(deg)), degs);
    var mFin = Mechanism(deg => KineticUnits.FinScreen(3, 2.4, 8, 0.3, 0.045, 1, 0.12, 0.08, KineticUnits.NormalFin(deg, 1)), degs);
    Check("the louvre's linkage stays a linkage from -60 to 75 degrees: rigid cranks with their pins on the rod, the rod pinned to a piston that runs in a hinged cylinder (overhead, slat and fin)", mOvr.Ok && mSlat.Ok && mFin.Ok, mOvr.Why + mSlat.Why + mFin.Why);
    Check("the actuator's stroke over that range is what the crank arm asks for: a few cm to the side of the rod's own axis, and at most 2 x the crank radius plus the pin stub", mOvr.StrokeM > 0.02 && mOvr.StrokeM < 0.2 && mSlat.StrokeM > 0.005 && mFin.StrokeM > 0.005, $"(overhead {mOvr.StrokeM * 1000:0} mm, slat {mSlat.StrokeM * 1000:0} mm, fin {mFin.StrokeM * 1000:0} mm)");
    ...
    // the masts are telescopes: three stages that nest, share the extension, and reach the fabric's corner; fully in, the mast is one stage tall, so a storm cannot lower the sail below that
    var telFull = 3.5 + 0.15 - KineticUnits.CarriageTopM(0.10);
    var tele = KineticUnits.Sail(false, 6, 5, 1.0, 0, 3.5, 0.14, 0.10, 0.5, 1.4, 0.3, 0.30, 3, 0.15, telFull);
    var firstMast = tele.Bars.First(x => x.Role == "mast");
    var stagesOfOne = tele.Bars.Where(x => x.Role == "mast" && Near(x.P0.X, firstMast.P0.X, 1e-9) && Near(x.P0.Y, firstMast.P0.Y, 1e-9)).OrderBy(x => x.P0.Z).ToList();
    var stageLen = KineticUnits.TelescopeStageM(telFull, 3, 0.15);
    Check("a telescoping mast is three nested stages of equal length: each 16 mm narrower than the one below, each keeping 0.15 m inserted, the top stage ending at the fabric's corner", stagesOfOne.Count == 3 && stagesOfOne.All(x => Near((x.P1 - x.P0).Length, stageLen, 1e-9)) && Near(stagesOfOne[0].SizeU - stagesOfOne[1].SizeU, KineticUnits.TelescopeStepM, 1e-9) && Near(stagesOfOne[1].SizeU - stagesOfOne[2].SizeU, KineticUnits.TelescopeStepM, 1e-9) && Near(stagesOfOne[0].P1.Z - stagesOfOne[1].P0.Z, 0.15, 1e-9) && Near(stagesOfOne[1].P1.Z - stagesOfOne[2].P0.Z, 0.15, 1e-9) && Near(stagesOfOne[2].P1.Z, 3.5 + 0.15, 1e-9) && !stagesOfOne.Skip(1).Any(x => !x.Dynamic), $"(stage {stageLen:0.000} m)");
    var telStorm = SailMechanics.Analyse(d0, false, 6, 5, 3.5, 0, 1000, 0, sailStates);
    var tornDown = KineticUnits.Sail(false, 6, 5, 0.5, 0, telStorm.StormHeightM, 0.14, 0.10, 0.5, 1.4, 0, 0.30, 3, 0.15, telFull);
    var lowestMast = tornDown.Bars.Where(x => x.Role == "mast").OrderBy(x => x.P1.Z).First();
    Check("the storm height is the collapsed mast, not the 0.6 m entered: 3 stages of 1.26 m leave the sail at about 1.44 m, and the storm plan's masts really are that low (every stage above its base's carriage, the fabric's corners on the tops)", telStorm.MastStages == 3 && telStorm.StormHeightM > 1.3 && telStorm.StormHeightM < 1.6 && telStorm.StormHeightM > d0["sail_storm_height_m"] && tornDown.Bars.Where(x => x.Role == "mast").Max(x => x.P1.Z) <= telStorm.StormHeightM + 1e-9 && Near(tornDown.Surfaces[0].A.Z, telStorm.StormHeightM, 1e-9) && lowestMast.P0.Z >= KineticUnits.CarriageTopM(0.10) - 1e-9, $"({telStorm.StormHeightM:0.00} m, stage {telStorm.MastStageM:0.00} m, {telStorm.MastMassKgEach:0} kg a mast)");
    var rigid = SailMechanics.Analyse(new LouvreDesign(new Dictionary<string, double> { ["mast_stages"] = 1 }), false, 6, 5, 3.5, 0, 1000, 0, sailStates);
    Check("one stage is a rigid tube: the storm height is what was entered (0.6 m) and the mast is one bar", rigid.MastStages == 1 && Near(rigid.StormHeightM, d0["sail_storm_height_m"], 1e-9) && KineticUnits.Sail(false, 6, 5, 1.0, 0, 3.5, 0.14, 0.10, 0.5, 1.4, 0.3).Bars.Count(x => x.Role == "mast") == 4);
