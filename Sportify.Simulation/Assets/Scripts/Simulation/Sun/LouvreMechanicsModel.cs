...
namespace Sportify.Simulation.Sun
{
    ...
    public sealed class LouvreDesign
    {
        public static readonly LouvreInputDef[] Inputs =
        {
            ...
            new LouvreInputDef { Key = "mast_stages", Label = "Sail mast: telescope stages (1 = a rigid tube)", Unit = "-", Default = 3, Status = "assumed", Reference = "a mast that lowers itself is a telescope, each stage sliding inside the one below: a single tube cannot shorten from 3.5 m to 0.4 m. The storm height cannot be below the collapsed mast" },
            new LouvreInputDef { Key = "mast_overlap_m", Label = "Sail mast: length two stages stay inserted at full extension", Unit = "m", Default = 0.15, Status = "assumed", Reference = "about one tube diameter, for the stiffness of the joint" },
