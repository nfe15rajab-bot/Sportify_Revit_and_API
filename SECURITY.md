...
- **Uninstall**: the installer records every file it copies (`%APPDATA%\Sportify\install.json`), keeps a copy of itself, and registers *Sportify for Revit 2025* under Windows' Installed apps. `Sportify_Revit.exe --uninstall [--dry-run] [--purge] [--yes]` removes exactly those files, the manifest and the folders that are left empty, never another add-in's files and never the person's Sportify folder of layouts and reports; it stops if Revit is running and keeps its record when a file cannot be removed so that it can be run again (`Tools/InstallerCheck`). `--purge` also removes the logs, the settings and the generated families.
