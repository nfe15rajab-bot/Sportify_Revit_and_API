...
| The Kinetics buttons work as a person uses them | `KineticsWalkthrough` (env `SPORTIFY_KINETICS_WALKTHROUGH`): the ribbon's own commands posted to a live Revit 2025 on a real layout exported by the web app (a 67.5 x 16.3 m roof): Generate Family, Choose Family, Import Analysis Adaptation for an overhead louvre, a sail (auto and triangle), a slat screen on a selected wall, a fin screen on a selected railing, and Simulate (SOLIDWORKS); every dialog answered and written down, what each command placed read back from the model and from what the web app is given: no failures, 3.3 minutes. Not exercised: the roller fence (needs the Unity Ball Trajectory analysis), Record Isolated Video (drives the real Unity project) | machine (live Revit) |
