    ...
    internal static class RibbonLayout
    {
        ...
        public static readonly IReadOnlyList<RibbonPanelSpec> Panels = new[]
        {
            ...
            new RibbonPanelSpec("BIM & Documentation", new RibbonEntry[]
            {
                ...
                new RibbonButtonSpec("SportifyWorksets", "Worksets", "WorksetsCommand", "Sorts the model onto the Sportify worksets that mirror the Push to Sportify drop-down (Sportify Roof, Structure, Entries, Openings, Edge, Drains, Equipment, Slab): every roof, floor, grid, column, beam, bearing wall, stair, ramp, door, lift, shaft, railing, drain and piece of equipment is recognised by its category and family name and goes to the workset of its item, so the model can be pushed by workset. Optional: a checkbox activates worksets (turns worksharing on); elements on a default workset move, elements on another workset are flagged and selected, not moved, unless you choose that.", "worksets"),
                new RibbonButtonSpec("ApplySportifyTemplate", "Apply Sportify\nTemplate", "ApplySportifyTemplateCommand", "Makes the Sportify templates in this project, in German (HOAI Leistungsphasen, AVA lists) or in English (the same structure with English names; you choose): view templates (a roof plan per phase, pieces by kind, zones by build-up, circulation, axonometric, sections), their filters, schedules (AVA/QTO, DIN 277 areas, DIN 276 cost groups) and sheets on the German Plankopf, and, when the project holds a Sportify layout, the views of it on those sheets. A project in imperial units is converted to the German template's meters, m² and m³ first. Nothing of yours is changed.", "template_apply"),
                new RibbonPulldownSpec("PullTemplates", "More\nTemplates", "The Sportify template in one language directly, and Revit's own built-in templates (the German BIM template's and the English one's), hidden until you ask.", "templates", new[]
                {
                    new RibbonButtonSpec("ApplySportifyTemplateDe", "Sportify Template (Deutsch)", "ApplySportifyTemplateDeCommand", "The Sportify template with the German names of Revit's German BIM template: Leistungsphasen 1-4, Dachaufsicht, AVA-Listen, Flächen nach DIN 277, Kostengruppen nach DIN 276.", "template_apply"),
                    new RibbonButtonSpec("ApplySportifyTemplateEn", "Sportify Template (English)", "ApplySportifyTemplateEnCommand", "The same structure with English names (Roof Plan, QTO lists, area and cost group schedules); the DIN norms keep their numbers and the sheets keep the German Plankopf.", "template_apply"),
                    new RibbonButtonSpec("HideRevitTemplates", "Hide Revit Templates", "HideRevitTemplatesCommand", "Removes the view templates, views, sheets, schedules and filters that Revit's German or English template brought and that nothing of yours uses, so the project browser and the template lists show only Sportify's. They can be brought back.", "template_hide"),
                    new RibbonButtonSpec("ShowRevitTemplates", "Show Revit Templates", "ShowRevitTemplatesCommand", "Copies Revit's own view templates, filters and schedules back into this project from the template it came from (German BIM or English multi-discipline).", "template_show"),
                }),
            }),
