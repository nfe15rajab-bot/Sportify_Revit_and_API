...
1. *Simple / Advanced as a view, not a lock.* One switch in the web app and in the Revit ribbon (Revit can show and hide ribbon buttons and panels at run time). Simple = the eight buttons and four tabs of the main path; Advanced = everything. Stored in `%APPDATA%\Sportify\settings.json` so Revit and the web app agree. **Started (branch `ux/profile`, both repositories):** the web app's *Profile* tab next to New Session keeps the view (Simple = Overview, Site, Sport, Combine, Analysis + Deliverables, Save Session, New Session, Profile; Advanced = everything), role, theme and the person's name and photo; it is saved at once in the browser, in `settings.json` under `"profile"` (newer copy wins) and in the *Profile* folder of the Sportify folder. **Not yet:** the choices marked as defaults in `profileCore.js` awaiting review.
2. *Capability detection instead of questions.* The add-in already knows whether Unity, SOLIDWORKS and Chrome are there: hide "Simulate" without SOLIDWORKS, offer PDF charts without Unity. **Built (branch `ux/capabilities`, both repositories):** `SportifyCapabilities` + `CapabilityProbes` (`GET /capabilities`), the analysis dialogs no longer ask "do you have Unity?", the ribbon hides what cannot work and, in the Simple view, the deeper buttons (`RibbonVisibility`, applied by `RibbonApplier` on Revit's thread), the Profile tab's "This computer" section, the installer's last page says Chrome or the default browser. Tested without Revit (AddinCheck 28 checks over every view × tools, ContractCheck the endpoint); **still to see in a live Revit:** that `RibbonItem.Visible` / `RibbonPanel.Visible` really hide the drop-down items and panels, that a task dialog with only the PDF link (no link 1) shows, and the refresh after a profile change.
